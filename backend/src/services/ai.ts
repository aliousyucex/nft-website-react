import { Card, CardType, CARD_ADVANTAGES } from '../types/card';
...
export class AIService {
   ...
   */
  selectCard(hand: Card[], opponentLastCard?: Card, currentScore?: { ai: number; player: number }): Card {
    if (hand.length === 0) {
    ...
    logger.debug('AI selecting card', {
      ...
      opponentLastCard: opponentLastCard ? `${opponentLastCard.type}_${opponentLastCard.value}` : 'none',
      score: currentScore,
    });
    ...
    const weightedCards = hand.flatMap(card => 
      Array(weights[card.value] || 1).fill(card)
    );
