...
const ipRateLimits = new Map<
  string,
  {count: number; lastReset: number; bannedUntil: number | null}
>();
...
export const rateLimitMiddleware = async (socket: Socket, next: (err?: Error) => void) => {
  ...
  try {
    ...
    const ipData = ipRateLimits.get(ip) || {count: 0, lastReset: now, bannedUntil: null};
...
export const checkAddressRateLimit = (address: string): boolean => {
  const now = Date.now();
  const addressData = addressRateLimits.get(address) || {
    count: 0,
    lastReset: now,
    bannedUntil: null,
  };
...
export const cleanupExpiredBans = async () => {
  try {
    await db.deleteFrom('ban_list').where('banned_until', '<', new Date()).execute();
